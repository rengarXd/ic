			...
			factory(window["H"] = {
				...
				LunarDate : {
					...
					e2c : function() {
						...
						for ( m = 0; ; m++) {
							k = (this.CalendarData[m] < 4095) ? 11 : 12;
							for ( n = k; n >= 0; n--) {
						...
						this.Day = total;
						if (k == 12) {
							if (this.Month == Math.floor(this.CalendarData[m] / 65536) + 1) {
				...
				//@data:{values:{}}
				userPrefs : function(callback, url, data) {
					data = data ? data : {};
					$$apicloud.getPrefs(function(ret, err) {
						var userinfoobj = eval('(' + ret.value + ')');
						$$apicloud.ajax({
							url : window.serverUrl + '/' + url + '/token/' + userinfoobj.token + '/user_id/' + userinfoobj.id,
							data : data
						}, function(ret, err) {
							if ($$com.isFunction(callback)) {
								callback(ret, err, userinfoobj);
							}
						});
					}, 'userinfo');
				},
